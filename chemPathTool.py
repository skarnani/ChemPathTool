...
class ChemPathTool:
    ...
    def findObj(self,event):
        closestObjs = self.draw.find_closest(self.startx,self.starty)
        global nodeMap
        obj = None
        if (len(closestObjs)>0):
            ...
            for spec in nodeMap.keys():
                ...
                if node not in self.lockedNodes:
                    if node.contains(objID):
                        if r2dist(node.xc,node.yc,cx,cy) < node.radius: obj = node
        return obj
...
if __name__ == '__main__': 
    ...
    if len(sys.argv) == 1:
        ...
        # If you want to call this function from the command line and
        # pass it a set of input data, the nodes and edges need to be
        # "pickled" in the same order as it is "unpickled" here:
        #
        # f = open(cplFile,'w')
        # p = cPickle.Pickler(f)
        # p.dump(nodes)
        ...
        # f.close()
        #
        # where the node and edge structures are objects such as the ones above
        #
        infile = sys.argv[1]
        f = open(infile,'r')
        u = cPickle.Unpickler(f)
