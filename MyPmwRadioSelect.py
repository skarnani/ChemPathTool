...
class MyRadioSelect(Pmw.MegaWidget):
	...
	if self['orient'] not in ('horizontal', 'vertical'):
        ...
        self._maxcr = self['maxcr']

	# Check keywords and initialise options.
	self.initialiseoptions(MyRadioSelect)
	...
	if type(index) == types.IntType:
		...
		raise ValueError, 'MyRadioSelect has no buttons'
	else:
	...
	if self['orient'] == 'horizontal':
            self._ncol = self._maxcr
	    self._nrow = int (len(self._buttonList) / self._maxcr) + 1
        else:
            self._nrow = self._maxcr
	    self._ncol = int (len(self._buttonList) / self._maxcr) + 1

	if self['orient'] == 'horizontal':
	    col = len(self._buttonList) / self._ncol
	    row = len(self._buttonList)  - (col-1)*self._maxcr
	    button.grid(column=col, row=row, padx = self['padx'],
	    ...
	    row = len(self._buttonList) / self._nrow
	    col = len(self._buttonList)  - (row-1)*self._maxcr
	    button.grid(column=col, row=row, padx = self['padx'],
	...
	self._buttonList.append(componentName)
        ...
        if _newInit == 'selected':
            if self._singleSelect:
                self.selection = componentName
                if self['buttontype'] == 'radiobutton':
                    widget = self.component(componentName)
                    widget.select()
                else:
                ...
                # Multiple selections
                widget = self.component(componentName)
                if componentName in self.selection:
                    if self['buttontype'] == 'checkbutton':
                        widget.deselect()
                    else:
                        widget.configure(relief='raised')
                    self.selection.remove(componentName)
                else:
                    if self['buttontype'] == 'checkbutton':
                        widget.select()
                    else:
                        widget.configure(relief='sunken')
                    self.selection.append(componentName)
